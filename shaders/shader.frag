...
void main() {
	...
	vec3 specular;
	if (refl.x > 0.5f) {
		specular = specColor * pow(max(dot(R,V), 0.0f), specPower); // Ho sostituito specColor con diffColor
	} else {
		specular = vec3(0.0f);
	}

	// Hemispheric ambient
	vec3 ambient  = 0.35f * (vec3(0.1f, 0.1f, 0.1f) * (1.0f + N.y) + vec3(0.0f, 0.0f, 0.1f) * (1.0f - N.y)) * diffColor;
